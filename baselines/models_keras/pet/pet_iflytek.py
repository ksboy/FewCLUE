...
def load_data(filename, set_type="train"): # 加载数据
    ...
    with open(filename, encoding='utf-8') as f:
        for i, l in enumerate(f):
            l = json.loads(l)
            label_des=l['label_des'] if 'label_des' in l else "音乐"
            # 如果想尝试每个类别只用一个样本
...
def draw_acc(acc_list):
    ...
    ax.legend()
    plt.savefig(os.path.join(output_dir, "pet_iflytek.svg")) # 保存为svg格式图片，如果预览不了svg图片可以把文件后缀修改为'.png'
...
if __name__ == '__main__':
    ...
    if training_type == "few-shot":
        ...
        val_data = load_data('../../../datasets/iflytek/dev_0.json')
        test_data = load_data('../../../datasets/iflytek/test.json')
